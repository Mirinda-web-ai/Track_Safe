...
class TrackSafeSystem:
    ...
    def view_unknown_workers(self):
        """View and manage unknown workers captured by the system"""
        unknown_folder = "Unknown_Workers"
        
        if not os.path.exists(unknown_folder):
            print("📁 No unknown workers folder found.")
            print("Unknown workers will be saved automatically when detected during monitoring.")
            return
        
        unknown_dirs = [d for d in os.listdir(unknown_folder) 
                    if os.path.isdir(os.path.join(unknown_folder, d)) and d.startswith('Unknown_')]
        
        if not unknown_dirs:
            print("📁 No unknown workers found yet.")
            print("Start monitoring to automatically capture unknown workers.")
            return
        
        print(f"\n👻 === UNKNOWN WORKERS DETECTED ({len(unknown_dirs)}) ===")
        print("=" * 80)
        
        for i, unknown_dir in enumerate(unknown_dirs, 1):
            unknown_path = os.path.join(unknown_folder, unknown_dir)
            images = [f for f in os.listdir(unknown_path) if f.lower().endswith(('.jpg', '.jpeg', '.png'))]
            
            print(f"\n{i}. Unknown Worker: {unknown_dir}")
            print(f"   📸 Images captured: {len(images)}")
            print(f"   📂 Location: {unknown_path}")
            
            if images:
                first_image_path = os.path.join(unknown_path, images[0])
                try:
                    ...
                    if img is not None:
                        ...
                        cv2.putText(img, f"Unknown Worker {i}", (10, 30), 
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                        cv2.putText(img, "Press any key to continue", (10, img.shape[0] - 10), 
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
                        
                        cv2.imshow(f"Unknown Worker {i}", img)
                        cv2.waitKey(0)
                        cv2.destroyAllWindows()
                except:
            ...
            while True:
                choice = input(f"\n   What would you like to do with Unknown Worker {i}?\n"
                            f"   1. Register as new worker\n"
                            f"   2. Ignore (keep images)\n"
                            f"   3. Delete images\n"
                            f"   4. Skip to next\n"
                            f"   Enter choice (1-4): ").strip()
                
                if choice == '1':
                    print(f"\n🔄 Starting registration for Unknown Worker {i}...")
                    print("Note: You'll need to capture new images during registration.")
                    input("Press Enter to start registration process...")
                    self.register_new_worker()
                    break
                elif choice == '2':
                    print(f"✅ Images for Unknown Worker {i} will be kept for future reference.")
                    break
                elif choice == '3':
                    confirm = input(f"⚠️  Are you sure you want to delete all images for Unknown Worker {i}? (y/n): ").lower()
                    if confirm == 'y':
                        try:
                            import shutil
                            shutil.rmtree(unknown_path)
                            print(f"🗑️  Deleted images for Unknown Worker {i}")
                        except Exception as e:
        ...
        print(f"\n✅ Unknown workers review completed.")
        print(f"📁 Unknown workers folder: {os.path.abspath(unknown_folder)}")
    ...
    def show_main_menu(self):
        ...
        while True:
            ...
            print("4. View Unknown Workers")
            print("5. Reload Database")
            print("6. Exit Track_Safe System")
            
            choice = input("\nEnter your choice (1-6): ").strip()
            
            if choice == '1':
                print("🚀 Starting Track_Safe monitoring...")
                print("System will assign TEMP_001, TEMP_002 IDs to detected persons")
                print("Press 'q' during monitoring to stop")
...
class EnhancedTrackSafeSystem(TrackSafeSystem):
    ...
    def run_integrated_monitoring(self):
        """Fixed monitoring with proper person detection and bounding boxes"""
        print("\n🔄 Initializing Track_Safe monitoring system...")
        
        # Initialize models
        if not self.initialize_ppe_models():
            print("❌ Cannot start monitoring - PPE models not loaded")
            print("Make sure 'best (1).pt' and 'yolov8n.pt' are in your project folder")
        ...
        print("🚀 Track_Safe monitoring started")
        print("Features active:")
        print("- Person detection with bounding boxes")
        print("- Temporary ID assignment (TEMP_001, TEMP_002, etc.)")
        ...
        try:
            while True:
                ...
                # STEP 1: Detect people first
                person_boxes = []
                try:
                    person_results = self.person_model(frame, conf=0.25, verbose=False)  # Lower confidence
                    for r in person_results:
                        if hasattr(r, 'boxes') and r.boxes is not None:
                            for box in r.boxes:
                                if int(box.cls[0]) == 0:  # Person class
                                    x1, y1, x2, y2 = map(int, box.xyxy[0])
                                    # Minimum size filter to avoid tiny detections
                                    if (x2-x1) > 30 and (y2-y1) > 30:
                                        person_boxes.append((x1, y1, x2, y2))
                                        print(f"✓ Person detected at ({x1},{y1},{x2},{y2})")  # Debug
                except Exception as e:
                    print(f"Person detection error: {e}")
                
                # STEP 2: Update person tracking
                self.person_tracker.update_tracks(person_boxes, frame)
                
                # STEP 3: Detect PPE equipment
                equipment_status = {item: {'worn': False, 'held': False} for item in self.REQUIRED_SAFETY_ITEMS}
                face_protector_status = {'worn': False, 'held': False}
                non_safety_detected = False
                
                try:
                    ppe_results = self.ppe_model(frame, conf=0.3, verbose=False)  # Lower confidence
                    for r in ppe_results:
                        if hasattr(r, 'boxes') and r.boxes is not None:
                            for box in r.boxes:
                                x1, y1, x2, y2 = map(int, box.xyxy[0])
                                ...
                                if cls < len(self.classNames):
                                    ...
                                    if conf > 0.3:  # Lower threshold
                                        ...
                                        # Check if equipment is worn by any tracked person
                                        for person_id, person_data in self.person_tracker.tracked_persons.items():
                                            ...
                                            if overlap > item_area * 0.2:  # Lower overlap threshold
                                                is_worn = True
                                                break
                ...
                # STEP 4: Draw tracked persons with IDs and status
                tracked_count = 0
                current_time = time.time()
                ...
                for person_id, person_data in self.person_tracker.tracked_persons.items():
                    ...
                    cv2.rectangle(frame, (x1, y1), (x2, y2), display_info['color'], 3)
                    
                    # Draw person info above box
                    info_text = f"{display_info['name']} ({display_info['id']})"
                    cv2.putText(frame, info_text, (x1, max(20, y1 - 30)), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, display_info['color'], 2)
                    
                    # Draw tracking status
                    status_text = display_info['status']
                    cv2.putText(frame, status_text, (x1, max(40, y1 - 10)), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, display_info['color'], 1)
                    ...
                    # Draw compliance status below box
                    if all_worn:
                        status_msg = 'COMPLIANT - All PPE detected'
                        status_color = (0, 255, 0)
                    else:
                        missing_count = sum(1 for item in self.REQUIRED_SAFETY_ITEMS if not equipment_status[item]['worn'])
                        status_msg = f"NON-COMPLIANT - Missing {missing_count} items"
                        status_color = (0, 0, 255)
                    
                    cv2.putText(frame, status_msg, (x1, min(frame.shape[0] - 10, y2 + 20)), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, status_color, 1)
                    
                    # Log worker status if confirmed
                ...
                # STEP 5: Alert system - beep only for non-compliant workers
                ...
                # STEP 6: Display system info (always visible)
                cv2.rectangle(frame, (0, 0), (frame.shape[1], 40), (0, 0, 0), -1)
                system_info = f"Track_Safe | Database: {self.database_name} | Workers: {len(self.workers_data)} | Active: {tracked_count}"
                cv2.putText(frame, system_info, (10, 25), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            ...
            print("Check if your YOLO model files are correct:")
            print("- best (1).pt should be your PPE detection model")
            print("- yolov8n.pt should be in the same folder")
        finally:
            cap.release()
            cv2.destroyAllWindows()
